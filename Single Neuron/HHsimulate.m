...
for i=1:Ntme-1
    ...
    V(i+1)=V(i)+dt*(1/Cm)*(I(i)-(gK*(N(i)^4)*(V(i)-Vk))-(gNa*(M(i)^3)*H(i)*(V(i)-Vna))-(gL*(V(i)-Vl)))+dV(i);
...
jj=iii+(n*iip);
LimDist=sqrt(((Vstd-V(jj)).^2)+((Mstd-M(jj)).^2)+((Nstd-N(jj)).^2)+((Hstd-H(jj)).^2));
StabDist=sqrt(((Vo-V(jj))^2)+((Mo-M(jj))^2)+((No-N(jj))^2)+((Ho-H(jj))^2));
